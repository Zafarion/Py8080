...
#screen = pygame.display.set_mode((682, 512))
...
screen.blit(font.render('SV version with wide screen and background image', True, turquoise), (0, 180))
screen.blit(font.render('TV & Midway versions with wide screen and background image', True, red), (0, 210))
screen.blit(font.render('CV version with wide screen and background image', True, white), (0, 240))
pygame.display.flip()
...
# Extender RAM em mais 0x2400 bytes (dúvidas procurar memory map do space invaders)
    ...
    match ram[IPC()]:
        ...
        case 0xD3: # OUTP A
            match ram[IPC()]:
                ...
                case _:
                    crashed = True
                    print("Undefined output port: ", hex(ram[PC - 1]))

            if (OUT_PORT3 != last_out_port3):
                if (OUT_PORT3 & 0x1) and not (last_out_port3 & 0x1):
                    ufo_highpitch.play(-1)
                elif not (OUT_PORT3 & 0x1) and (last_out_port3 & 0x1):
                    ufo_highpitch.stop()
                elif (OUT_PORT3 & 0x2) and not (last_out_port3 & 0x2):
                    invaderkilled.play()
                elif (OUT_PORT3 & 0x4) and not (last_out_port3 & 0x4):
                    explosion.play()
                elif (OUT_PORT3 & 0x8) and not (last_out_port3 & 0x8):
                    shoot.play()
            ...
            if (OUT_PORT5 != last_out_port5):
                if (OUT_PORT5 & 0x1) and not(last_out_port5 & 0x1):
                    fastinvader3.play()
                elif (OUT_PORT5 & 0x2) and not(last_out_port5 & 0x2):
                    fastinvader4.play()
                elif (OUT_PORT5 & 0x4) and not(last_out_port5 & 0x4):
                    fastinvader1.play()
                elif (OUT_PORT5 & 0x8) and not(last_out_port5 & 0x8):
                    fastinvader2.play()
                elif (OUT_PORT5 & 0x10) and not(last_out_port5 & 0x10):
                    ufo_lowpitch.play()
        ...
        case 0xDB: # INP A
            ...
            match ram[IPC()]:
                ...
                case _:
                    crashed = True
                    print("Undefined input port: ", hex(ram[PC - 1]))

             # Resetar portas I/O
            IN_PORT1 = IN_PORT1 & (~(0x1 | 0x2 | 0x4 | 0x10 | 0x20 | 0x40))
            IN_PORT2 = IN_PORT2 & (~(0x4 | 0x10 | 0x20 | 0x40))
